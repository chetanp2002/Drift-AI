...
def _llm_text(system_prompt: str, user_prompt: str) -> str:
    ...
    # Safely extract text from Gemini's list response
    if hasattr(response, "text") and response.text:
        return response.text
    if isinstance(response.content, list):
        return "".join(
            p if isinstance(p, str) else p.get("text", "") if isinstance(p, dict) else getattr(p, "text", "")
            for p in response.content
        )
    return str(response.content)

def _json_from_llm(text: str) -> dict[str, Any]:
    '''Extract the complete JSON object returned by the model'''
    start = text.find("{")
    end = text.rfind("}")  # Use rfind to capture nested objects
...
def supervisor_agent(state: TravelState):
    ...
    guardrail_prompt= f"""
Determine weather the following request belongs to travel planning or travel
information. Valid requests can include destinations, flights, hotels, weather, budgets, 
visas, transportation, sightseeing, food, packing, or itineraries

Block clearly unrelated requests and requests asking for jarmful or illegal instructions
Do not block a valid travel request marely because some details are missing
...
{query}
"""
    # Fail to open on model errors so a remporary JSON-format issue does not break the original travel-planning behavior
    try:
        ...
        allowed= bool(guardrail_result.get('reason', '')).strip()
        llm_calls +=1
    except Exception as exc:
        print('fGuardrail fallback used: {exc}')
        allowed= True
        guardrail_reason= 'Fuardrail validation fallback allowed the request'

    if not allowed:
        reason= guardrail_reason or (
            'TripMate AI can only help with travel-planning requests'
            'Please ask about destination, flight, hotel, weather, budget or itinerary'
        )
        return {
...
def budget_agent(state: TravelState):
    prompt= f"""
Analyze wheather this trip is realistic for the user's budget
...
If exact live prices are unavailable, clearly label estimated as approximate
"""
...
def _to_text(val) -> str:
    ...
    if isinstance(val, list):
        return "".join(part if isinstance(part, str) else part.get("text", "") if isinstance(part, dict) else getattr(part, "text", "") for part in val)
    return str(val or "")
